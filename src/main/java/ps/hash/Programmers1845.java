...
/**
 * 프로그래머스
 * - 코딩테스트 연습 - 해시 - 폰켓몬 (https://school.programmers.co.kr/learn/courses/30/lessons/1845)
 *
 * 문제 설명
...
public class Programmers1845 {
    ...
    public static void main(String[] args) {
        final int[] nums1 = {3,1,2,3};
        final int[] nums2 = {3,3,3,2,2,4};
        final int[] nums3 = {3,3,3,2,2,2};
    ...
    public static int solution(int[] nums) {
        final Set<Integer> numberSet = new HashSet<>(); // 중복 제거를 위한 Set
        for (int num : nums) {
            numberSet.add(num);
        }

        final int n = nums.length / 2;
        final int size = numberSet.size();

        if (size >= n) return n; // 중복 제거 마릿수가 N/2보다 크다면 N/2 반환

        return size; // 중복 제거 마릿수가 N/2보다 작다면 중복 제거 수 반환
    }
